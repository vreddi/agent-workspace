...
import type { EntityId, TerrainCell, World, WorldObject } from './types.js'
...
export function removeTerrain(world: World, position: GridPosition): World {
...
export function isCellStandable(world: World, position: GridPosition): boolean {
  ...
  return !getObjectsAt(world, position).some((object) => object.blocksMovement)
}

export function isCellBlocked(world: World, position: GridPosition): boolean {
