...
describe('parseMap', () => {
  it('parses ground, props, and markers', () => {
    const map = parseMap(['....', 'T.~.', '.#1.'], LEGEND, VERDANT)
    ...
    expect(map.props).toEqual([{ prop: 'pine', x: 0, y: 1 }])
    expect(map.markers).toEqual({ spawn: { x: 2, y: 2, z: 0 } })
  })

  it('rejects unknown characters', () => {
    expect(() => parseMap(['?'], LEGEND, VERDANT)).toThrow(
  ...
  it('rejects props whose base does not fit', () => {
    // house-thatch is 3 wide with 2 base rows; anchored at top row it sticks out.
    expect(() => parseMap(['H...'], LEGEND, VERDANT)).toThrow(/does not fit/)
    expect(() => parseMap(['....', '..H.'], LEGEND, VERDANT)).toThrow(
      /does not fit/,
    )
  })

  it('rejects duplicate markers', () => {
    expect(() => parseMap(['1.1'], LEGEND, VERDANT)).toThrow(/duplicate marker/)
  })
})

describe('propBaseCells', () => {
  it('returns the blocking footprint anchored at the bottom-left', () => {
    const cells = propBaseCells({ prop: 'house-thatch', x: 2, y: 3 }, VERDANT)
  ...
  it('single-cell props block exactly their anchor', () => {
    expect(propBaseCells({ prop: 'pine', x: 5, y: 7 }, VERDANT)).toEqual([
      { x: 5, y: 7, z: 0 },
    ])
  })
})
