...
describe('positionsEqual', () => {
  it('compares by value across all three axes', () => {
    expect(positionsEqual({ x: 1, y: 2, z: 0 }, { x: 1, y: 2, z: 0 })).toBe(
      true,
    )
    expect(positionsEqual({ x: 1, y: 2, z: 0 }, { x: 2, y: 1, z: 0 })).toBe(
      false,
    )
    expect(positionsEqual({ x: 1, y: 2, z: 0 }, { x: 1, y: 2, z: 1 })).toBe(
      false,
    )
  })
})
