...
export function createGrid(config: GridConfig): GridConfig {
  if (!Number.isInteger(config.width) || config.width <= 0) {
    throw new Error(
      `grid width must be a positive integer, got ${config.width}`,
    )
  }
  if (!Number.isInteger(config.height) || config.height <= 0) {
    throw new Error(
      `grid height must be a positive integer, got ${config.height}`,
    )
  }
  if (!Number.isInteger(config.layers) || config.layers <= 0) {
    throw new Error(
      `grid layers must be a positive integer, got ${config.layers}`,
    )
  }
